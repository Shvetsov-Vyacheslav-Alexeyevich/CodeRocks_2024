<?
  ...
  if (!empty($_POST))
  {
    ...
    // ----------------------------------------------------------> Фильтрация по категориям (Доделать)
    ...
    // ----------------------------------------------------------> Данные для маршрутов (Славик) ---> Спросить
    ...
    // ----------------------------------------------------------> Оформление заказа (Доделать)
    else if ($_POST["form_type"] == "new_order") {
      echo json_encode(["status" => true]);
    }
    // ----------------------------------------------------------> Возвращает id карточки товара при нажатии на кнопку заказать ???
